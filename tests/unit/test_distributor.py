...
def test_unsupported_contract_version(distributor, caplog):
    distributor.w3.lido.nor_contracts[0].get_contract_version = Mock(return_value=2)
    ...
    distributor.w3.lido.nor_contracts[0].get_reward_distribution_state.assert_not_called()
...
def test_contract_not_ready_to_distribute(distributor, caplog):
    ...
    distributor.w3.lido.nor_contracts[0].get_reward_distribution_state = Mock(return_value=RewardDistributionState.DISTRIBUTED)
    distributor.execute(BlockData())
